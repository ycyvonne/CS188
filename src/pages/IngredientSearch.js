...
class IngredientSearch extends Component {
  ...
  addIngredientToTemp = newIngredient => {
    this.setState({
      selectedIngredients: [newIngredient].concat(
  ...
  removeIngredientFromTemp = ingredientToRemove => {
    ...
    this.setState({
      selectedIngredients: ingredientList.filter(
        ingredient => !_.some([ingredientToRemove], ingredient)
      ),
    })
  }
        ...
        {this.state.selectedIngredients.length == 0 &&
          this.state.searchText == '' && (
            <Header
              ...
              `}
            >
              Start typing names of ingredients to add to your list.
            </Header>
          )}
      </Page>
    )
